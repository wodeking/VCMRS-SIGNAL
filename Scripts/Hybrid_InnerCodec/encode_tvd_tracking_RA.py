...
test_id = f'TVD_tracking_RA_{timestamp}'
base_folder = "output"
...
ini_path = os.path.join(base_folder, 'TVD_tracking_ini', 'TVD_tracking_RA')
...
for video_id in config.seq_cfg.keys():
  ...
  for qp_idx, qp in enumerate(config.seq_cfg[video_id]):
