...
cpu_per_gpu_process = 5 # number of cpu process for each GPU process
...
# init environment
ini_path = os.path.join(base_folder, 'SFU_ini', 'SFU_AI')
...
tasks = []
log_files = []
for seq_id in sfu_config.seq_cfg_ai.keys():
  ...
  for qp_idx, enc_cfg in enumerate(sfu_config.seq_cfg_ai[seq_id]):
