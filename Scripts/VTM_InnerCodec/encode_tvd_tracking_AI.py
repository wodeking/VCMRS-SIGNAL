...
# setup ini file path
ini_path = os.path.join('output', 'working_dir', 'TVD_tracking_AI_ini')
os.makedirs(ini_path, exist_ok=True)

task_id = int(sys.argv[1]) - 1
seqs = list(config.seq_cfg_ai.keys())
num_seqs = len(seqs)
num_tasks = num_seqs * len(config.seq_cfg_ai['TVD-01_1'])
...
qp = config.seq_cfg_ai[video_id][qp_idx]
